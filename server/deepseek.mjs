...
function simulationPrompt(payload, evidence) {
  return [
    ...
    'unknowns.proof 必须写「拿到什么结果就算验证通过或被推翻」，是一个可观测的判据，例如「至少 1 人预付定金，或 10 人明确说不会付」。',
    '严禁在 proof 里写「无证据」「用户未提供」这类描述现状的话——那是 short 或 why 的内容。',
    '必须输出 3 个唯一 turningPoints，固定 id 为 sell-first、narrow-channel、cap-burn。',
    'turningPoints 的 riskReduction 表示优先级，取值范围 8-40，且必须落在三个不同区间：',
    '最该先做的那个取 34-40，第二个取 20-28，第三个取 10-16。三个值不得相同，也不得都取 40。',
    'obituary.cause 是死因短语，不超过 12 个字，例如「现金枯竭」「伪需求」；不要写成完整句子，否则会被截断。',
